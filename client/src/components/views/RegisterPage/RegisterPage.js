  ...
  const uploadImage = e => {
    ...
    if (files) {
      ...
      if (allowedTypes.indexOf(extension) < 0) {
        message.error(`'${files.type}' is not a supported format`)
      }
      if (files.size > 150000) {
        message.error(`'${files.name}' is too large, please pick a smaller file`)
      }
      else {
        var fd = new FormData();
          ...
          .catch((err) => {
            if (err) {
              switch (err.response.status) {
                case 400:
                  message.error('Sorry, Image invalid.');
                  break;
                case 404:
                  message.error('Sorry, Image not found');
                  break;
                case 500:
                  message.error('Sorry, upload file error, try again.');
                  break;
                default:
                  break;
              }
